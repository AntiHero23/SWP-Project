...
function StatisticKoi() {
  ...
  const fetchKoiStandard = async (koiFishID, selectedDate) => {
    ...
    try {
      console.log(
        `Fetching koi standard for koiFishID: ${koiFishID} on date: ${isoDate}`
      );
  ...
  const interpolate = (startDate, endDate, startValue, endValue) => {
    ...
    for (let i = 0; i <= daysBetween; i++) {
      ...
      const interpolatedValue =
        startValue + ((endValue - startValue) / daysBetween) * i;
  ...
  useEffect(() => {
    if (koiReport.length > 0 && koiStandard) {
      ...
      const existingDates = new Set(
        sortedReports.map((report) => report.updateDate)
      );
      ...
      let lastValue = sortedReports[0]
        ? sortedReports[0][selectedType]
        : koiStandard[`low${capitalize(selectedType)}`];

      // Interpolate from start of the month to the first report
      if (
        sortedReports.length > 0 &&
        dayjs(sortedReports[0].updateDate).isAfter(startOfMonth)
      ) {
      ...
      const interpolatedHigh = interpolate(
        startOfMonth,
        endOfMonth,
        highValueStart,
        highValueEnd
      ...
      fullChartData.forEach((dataPoint) => {
        ...
        const lowValueIndex = interpolatedLow.findIndex(
          (low) => low.updateDate === date
        );
        const highValueIndex = interpolatedHigh.findIndex(
          (high) => high.updateDate === date
        );
      ...
      const sortedFinalChartData = fullChartData.sort((a, b) =>
        dayjs(a.updateDate).diff(dayjs(b.updateDate))
      );
